...
# House caption style (kept in sync with scripts/vsl-render.py)
WORDS_PER_LINE = 3
FONT = "Arial"
BRAND = "līītt"
...
def band_margin(stem: str, video_h: int) -> int | None:
    ...
    margin = int(video_h - (y + h) + max(0, (h - FONT_SIZE * 1.4) / 2))
...
"""
    ...
    if group:
        lines.append(group)
    events = [
        f"Dialogue: 0,{_ass_time(g[0]['start'])},{_ass_time(g[-1]['end'])},Cap,,0,0,0,,"
        + " ".join(render_word(w["text"].strip()) for w in g)
        for g in lines
    ]
